...
export class SchemaManager {
   ...
   * @param atlassianClient The AtlassianClient instance to use for schema introspection.
   ...
   */
  constructor(atlassianClient: AtlassianClient, cacheDir?: string) {
    ...
    this.schemaPath = cacheDir
      ? join(cacheDir, 'atlassian-schema.graphql')
      : join(__dirname, 'schema', 'atlassian-schema.graphql');
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.error('Loaded Atlassian schema from cache');
    } catch {
      console.error(
        'No cached schema found, attempting to download from Atlassian...'
      );
      try {
        // Download schema from Atlassian
        this.schema = await this.downloadSchema();
        // Cache the downloaded schema
        await this.cacheSchema(this.schema);
        console.error('Downloaded and cached Atlassian schema');
      } catch (downloadError) {
        console.error(
          'Warning: Could not load Atlassian schema. Schema-driven features will be limited.',
  ...
  /**
   * Download schema from Atlassian using GraphQL introspection
   */
  private async downloadSchema(): Promise<string> {
    ...
    const result = await this.atlassianClient.executeQuery(introspectionQuery);
