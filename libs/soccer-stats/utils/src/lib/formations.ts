...
/**
 * Number of slots a formation defines for a position code.
 *
 * Formation codes are not unique across team sizes (e.g. "2-2" exists for
 * both 5v5 and 4v4), so `playersPerTeam` disambiguates when provided. When
 * it is omitted — or no formation of that size has the code — the maximum
 * count across all matching formations is used so that ambiguity never
 * rejects a valid lineup.
 *
 * @returns slot count (0 when the position is not part of the formation,
 *   e.g. sentinel codes like FIELD), or null when the formation code is
 *   unknown entirely. Callers doing capacity validation should skip the
 *   check for null and 0.
 */
export function getPositionSlotCount(
  formationCode: string,
  positionCode: string,
  playersPerTeam?: number,
): number | null {
  let candidates = ALL_FORMATIONS.filter((f) => f.code === formationCode);
  if (candidates.length === 0) {
    return null;
  }

  if (playersPerTeam !== undefined) {
    const sized = candidates.filter((f) => f.playersPerTeam === playersPerTeam);
    if (sized.length > 0) {
      candidates = sized;
    }
  }
  ...
  return Math.max(
    ...candidates.map(
      (f) => f.positions.filter((p) => p.position === positionCode).length,
    ),
  );
}
